...
@Component({
  selector: 'leverage-council-list',
  template: `
    <div class="container">
        <div class="row">
            <div class="col-md-12">
            ...
            <div class="panel panel-default"> 
                <div class="list-group">
                    ...
                    (click)="navigateToDetails(campaign)" class="list-group-item button-list-item">
                        ...
                        <span class="candidateName pull-left">{{ campaign.candidate_name }}</span>
                        <span class="pull-right">{{ campaign.campaigns[0].candidate_position }}</span>
                    </button>
                </div>
        </div>
    </div>`,
...
export class CouncilListComponent implements OnInit  {
    ...
    ngOnInit() {
        this.council = this.leverageConstants.COUNCIL_AT_LARGE;
        this.candidateListService.getCandidateDataStream()
            .subscribe(campaignsResponse => {
                campaignsResponse.forEach(campaign => {
                    if (campaign.campaigns[0].candidate_position === this.leverageConstants.COUNCIL_AT_LARGE) {
                        this.campaignLists.push(campaign);
                    }
                });
                // this.dispatchCampaigns(campaignsResponse);
            });
    }

    dispatchCampaigns(campaignsResponse: CampaignInfo[]) {
        this.store.dispatch({ 
